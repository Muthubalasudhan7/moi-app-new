    ...
    connection.query("select * from sample1.users where phone_no = ? and password = ?",[phone_no,password] ,function(error,results,fields){
        ...
        if (results.length > 0) {
                if (results[0].user_type === 1){
                    ...
                    const loggedInUserId = results[0].user_id;
                  ...
                  checkFunctionAssignment(user_id, (results, error) => {
                    ...
                    if (results && results.length > 0) {
                      ...
                      const writer_name = results[0].name;
                      req.session.writerUser = results[0];
                      req.session.loggedInUserId = loggedInUserId;
                      req.session.loggedInUserType = loggedInUserType;
                      req.session.writer_name = writer_name;
                  ...
                  connection.query(
                    "SELECT u.user_id, u.user_type, fd.function_id, fd.function_name, fd.function_date, fd.function_city, cd.customer_id, u.name " +
                    "FROM users u " +
                    "JOIN customer_details cd ON u.user_id = cd.user_id " +
                    "LEFT JOIN function_details fd ON cd.customer_id = fd.customer_id " +
                    "WHERE u.user_id = ? AND fd.is_delete = 0",
                    [user_id],
                    function (error, results) {
                      if (error) {
                        console.error("Error:", error);
                      ...
                      const functions = results.map(result => ({
                        ...
                        function_city: result.function_city,
                      ...
                      req.session.loggedInUserType = loggedInUserType;
                      req.session.name = name;
                      req.session.functions = functions;
                ...
                } else if (results[0].user_type === 4){
                  req.session.subAdminUser = results[0];
                  const name = results[0].name;
                  const loggedInUserId = results[0].user_id;
                  const loggedInUserType = results[0].user_type;
                  req.session.loggedInUserId = loggedInUserId;
...
// =========================================== ROUTE TO HANDLE THE FORGOT PASSWORD IN THE HOME PAGE ======================================================

// Define the route for sending the existing password via email
app.post('/checkAndSendEmail', (req, res) => {
  ...
  connection.query(sql, [email], function (error, results) {
    ...
    if (results.length > 0) {
      const existingPhoneNo = results[0].phone_no;
      const existingPassword = results[0].password;

      // Send the existing password to the user's email
      const mailOptions = {
        from: 'muthubalasudhan7@gmail.com', // Update with your email address
        to: email,
        subject: 'Existing Password',
        text: `Hi, Your existing phone no is: ${existingPhoneNo} \n Your existing password is: ${existingPassword} \n Thank You`,
      };
      ...
      res.status(401).json({ success: false, message: 'Email does not exist in the database' });
    }
  });
});
...
app.post('/writersubmit', (req, res) => {
    console.log(req.body);
      const {name,phone_no,email,gender,dateofbirth,writer_address,writer_city,altphonenumber,aadhaar_no} = req.body; // Assuming the field name is "customer_address"
    ...
    connection.query(userSql, userValues, function (err, userResult) {
      ...
      const writerSql = 'INSERT INTO writer_details (writer_dob, writer_address, writer_city, writer_alternate_phone_no, user_id, aadhaar_no) VALUES (?, ?, ?, ?, ?, ?)';
      const writerValues = [dateofbirth, writer_address, writer_city, altphonenumber, user_id, aadhaar_no];
...
app.post('/update-writer-data', (req, res) => {
    //console.log(req.body);
      const {writer_id,name,phone_no,email,gender,dateofbirth,writer_address,writer_city,altphonenumber,aadhaar_no,user_id} = req.body; // Assuming the field name is "customer_address"
...
app.delete('/delete-writer/:user_id', (req, res) => {
  ...
  connection.query('UPDATE users SET is_delete = 1 WHERE user_id = ?', [user_id], (error, result) => {
          if (error) {
...
// ================================ ROUTE TO HANDLE THE CUSTOMER REGISTRATION FORM =======================================================================
  ...
  app.post('/customersubmit', (req, res) => {
    console.log(req.body);
      const {name,phone_no,email,gender,dateofbirth,customer_address,customer_city,altphonenumber,customer_occupation,customer_husband_or_wifename,loggedInUserId} = req.body; 
      ...
      connection.beginTransaction(function (err) {
        ...
        // Insert data into the 'users' table, including the generated password
    const userSql = 'INSERT INTO users (name, phone_no, password, user_type, email, gender, inserted_by) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const userValues = [name, phone_no, password, user_type, email, gender, loggedInUserId ];
    connection.query(userSql, userValues, function (err, userResult) {
      if (err) {
        connection.rollback(function () {
          console.error('Error inserting user data:', err);
      ...
      const customerSql = 'INSERT INTO customer_details (customer_dob, customer_address, customer_city, customer_alternate_phone_no, user_id, customer_occupation, customer_husband_or_wifename) VALUES (?, ?, ?, ?, ?, ?, ?)';
      const customerValues = [dateofbirth, customer_address, customer_city, altphonenumber, user_id,customer_occupation, customer_husband_or_wifename];
...
app.post('/update-customer-data', (req, res) => {
  //console.log(req.body);
    const {customer_id,name,phone_no,email,gender,dateofbirth,customer_address,customer_city,altphonenumber,customer_occupation,customer_husband_or_wifename,user_id,loggedInUserId} = req.body;
    ...
    const updateCustomerFields = {
      ...
      customer_husband_or_wifename : customer_husband_or_wifename,      
    };
...
//=============================== ROUTE TO DELETE THE PARTICULAR CUSTOMER BASED ON THEIR PHONE NUMBER ===========================================
  app.delete('/delete-customer/:user_id', (req, res) => {
    const user_id = req.params.user_id;    
...
app.post('/insert-sub-admin-data', (req, res) => {
  console.log(req.body);
    const {name,phone_no,email,gender,dateofbirth,admin_address,admin_city,altphonenumber,aadhaar_no,loggedInUserId} = req.body; // Assuming the field name is "customer_address"
  ...
  connection.query(userSql, userValues, function (err, userResult) {
    ...
    // Insert data into the 'admin_details' table, linking it to the corresponding user
    const adminSql = 'INSERT INTO admin_details (admin_dob, admin_address, admin_city, admin_alternate_phone_no, aadhaar_no, user_id) VALUES (?, ?, ?, ?, ?, ?)';
    const adminValues = [dateofbirth, admin_address, admin_city, altphonenumber, aadhaar_no, user_id];
    connection.query(adminSql, adminValues, function (err, adminResult) {
      if (err) {
        connection.rollback(function () {
          console.error('Error inserting admin data:', err);
          res.status(500).json({ success: false, message: 'Failed to insert admin data' });
        });          
      }else {
          connection.commit(function (err) {
            if (err) {
              ...
              res.json({ success: true, message: 'Admin and user data inserted successfully' });
            }
          });
      }              
          });
...
// ============================== POST METHOD TO UPDATE THE SUB ADMIN DETAILS ============================================================================

app.post('/update-sub-admin-data', (req, res) => {
  console.log(req.body);
    const {admin_id,name,phone_no,email,gender,dateofbirth,admin_address,admin_city,altphonenumber,aadhaar_no,user_id} = req.body; // Assuming the field name is "customer_address"

    const updateAdminFields = {
      admin_dob : dateofbirth,
      admin_address: admin_address,
      admin_city : admin_city,
      admin_alternate_phone_no : altphonenumber,
      aadhaar_no : aadhaar_no,      
    };
    ...
    connection.beginTransaction(function (err) {
      ...
      // Update the writer_details table
      const writerSql = 'UPDATE admin_details SET ? WHERE admin_id = ?';
      connection.query(writerSql, [updateAdminFields, admin_id], function (err, result) {
          if (err) {
              connection.rollback(function () {
          ...
          connection.query(userSql, [updateUserFields, user_id], function (err, result) {
              ...
              connection.commit(function (err) {
                  if (err) {
                      ...
                      console.log('Both tables updated successfully1.');
                      res.json({ success: true, message: 'Admin and user data updated successfully' });
                  }
              });
          });
      });
...
//=============================== ROUTE TO DELETE THE PARTICULAR ADMIN BASED ON THEIR ADMIN_ID ===========================================
app.delete('/delete-admin/:user_id', (req, res) => {
  const user_id = req.params.user_id;    
...
connection.query('UPDATE users SET is_delete = 1 WHERE user_id = ?', [user_id], (error, result) => {
        if (error) {
            ...
            res.status(500).json({ success: false, message: "Failed to mark Admin as deleted" });
        } else {
            res.json({ success: true, message: "Admin marked as deleted successfully" });
        }
    });
});
...
app.post('/submit-visitor-form', (req, res) => {
  ...
  const {name,phone_no,profession,husband_wifename,address,townorvillage,visitor_city,ismaternaluncle,paymentamount,loggedInUserId,function_id,visitor_id} = req.body;
 
 if (!visitor_id || visitor_id === "") {

  const detailsSql  = 'INSERT INTO visitor_details (visitor_name, visitor_phone_no, visitor_profession, visitor_husband_or_wifename, visitor_address, visitor_city, visitor_town_or_village) VALUES (?, ?, ?, ?, ?, ?, ?)';
  const detailsValues  = [    
    name,
    phone_no,
    profession,
  ...
  connection.query(detailsSql, detailsValues, (err, result) => {
    if (err) {
      ...
      const paymentsSql = 'INSERT INTO visitor_payments (function_id,visitor_id, user_id,visitor_payment,is_maternal_uncle) VALUES (?, ?, ?, ?, ?)';
      const paymentsValues = [function_id,visitor_id,loggedInUserId,paymentamount,ismaternaluncle];
      ...
      'UPDATE visitor_details SET visitor_name=?, visitor_profession=?, visitor_husband_or_wifename=?, visitor_address=?, visitor_town_or_village=?, visitor_city=? WHERE visitor_id=?';
    ...
    connection.query(updateDetailsSql, updateDetailsValues, (err, updateResult) => {
      if (err) {
        console.error('Error updating data in visitor_details:', err);
        res.status(500).json({ error: 'An error occurred while processing your request' });
      } else {
        console.log('Data updated in visitor_details table');
...
app.post('/update-visitor-data', (req, res) => {
  console.log(req.body);
    const {user_id,function_id,visitor_id,visitor_payment_id,name,phone_no,husband_wifename,profession,ismaternaluncle,visitor_address,townorvillage,visitor_city,paymentamount} = req.body; 
    // console.log("user_id",user_id);
    // console.log("writer_id",writer_id);
    const updateVisitorFields = {
      ...
      visitor_town_or_village : townorvillage,         
    };

    const updatePaymentFields ={      
      visitor_payment : paymentamount,
      is_maternal_uncle : ismaternaluncle,
    }
    ...
    connection.beginTransaction(function (err) {
      ...
      connection.query(visitorSql, [updateVisitorFields, visitor_id], function (err, result) {
          ...
          // Update the users table
          const paymentSql = 'UPDATE visitor_payments SET ? WHERE function_id = ? AND user_id = ? AND visitor_id = ? AND visitor_payment_id = ?';
          connection.query(paymentSql, [updatePaymentFields, function_id,user_id,visitor_id,visitor_payment_id], function (err, result) {
              if (err) {
                  connection.rollback(function () {
                      console.error('Error updating user data:', err);
...
app.delete('/delete-visitor', (req, res) => {
  console.log(req.body);
  const {visitor_id,function_id,visitor_payment_id} = req.body; 

  connection.query('UPDATE visitor_payments SET is_delete = 1 WHERE visitor_id = ? and function_id = ? and visitor_payment_id = ?', [visitor_id,function_id,visitor_payment_id], (error, result) => {
        if (error) {
...
// ==================================== GET METHOD FOR PAGES AND DATA FOR THE REQUESTS =======================================================================================
...
function requireSubAdminAuth(req, res, next) {
  //console.log("Customer check",req.session.customerUser);

    if (req.session.subAdminUser) {
       return next();
    }
    res.redirect("/");
}
...
function ensureAuthenticated(req, res, next) {
  if (req.session.adminUser || req.session.writerUser || req.session.customerUser || req.session.subAdminUser) {
      // User is authenticated, allow access to the route
      return next();
  } else {
      // User is not authenticated, redirect to the login page
...
app.get("/agent", requireAdminAuth, function(req,res){
  res.sendFile(__dirname + "/agent_registration.html");
});

app.get("/admin-change-password", requireAdminAuth, function(req,res){
  res.sendFile(__dirname + "/admin_change_password.html");
});

app.get("/subAdminHome", requireSubAdminAuth, function(req,res){
  res.sendFile(__dirname + "/sub_admin_home.html");
});

app.get("/sub-admin-change-password", requireSubAdminAuth, function(req,res){
  res.sendFile(__dirname + "/sub_admin_change_password.html");
});

app.get("/sub-admin-customer", requireSubAdminAuth, function(req,res){
  res.sendFile(__dirname + "/sub_admin_customerpage.html");
});

app.get("/sub-admin-writer", requireSubAdminAuth, function(req,res){
  res.sendFile(__dirname + "/sub_admin_writerpage.html");
});
...
app.get("/get-customer-details-for-subadmin",requireSubAdminAuth, function (req, res) {
  const inserted_by = req.query.id; 

    // Query the database to fetch existing customer details
    connection.query("SELECT * FROM users u join customer_details cd on u.user_id = cd.user_id where u.is_delete = 0 and u.inserted_by = ? ORDER BY CASE WHEN cd.updated_at > u.updated_at THEN cd.updated_at ELSE u.updated_at END DESC",[inserted_by], function (error, results, fields) {
        if (error) {
            console.error("Error fetching customer details:", error);
            res.status(500).send("Internal server error");
        } else {
...
app.get('/api/get-customer-details-by-id',ensureAuthenticated, function (req, res) {
  const customer_id = req.query.id;  
  const loggedInUserType = req.session.loggedInUserType;
  ...
  switch (loggedInUserType) {
    ...
    case 4:
      // Agent user
      // Allow Agent to access all data
      break;
    default:
  ...
  connection.query(
      'SELECT * FROM users u join customer_details cd on u.user_id = cd.user_id WHERE cd.customer_id = ?',
      [customer_id],
      (err, results) => {
          if (err) {
...
app.get("/get-customers-count", ensureAuthenticated, function (req, res) {
  // Query the database to fetch existing customer details
  const loggedInUserType = req.session.loggedInUserType;
  ...
  connection.query(
    "SELECT COUNT(*) as totalCustomers from users where user_type = 3 and inserted_by = ? and is_delete = 0",[loggedInUserId],
...
app.get("/getWriterIds-for-subadmin",requireSubAdminAuth, function (req, res) {
  // Query the database to fetch available writer Ids
  //const function_date = parseInt(req.query.function_date);
  const function_date = req.query.functionDate;
  console.log(function_date);
...
app.get("/get-user-status",ensureAuthenticated, (req, res) => {
...
app.get("/get-single-customer-details-for-subadmin",requireSubAdminAuth, function (req, res) {
  //console.log("get single customer details function called ");
  const customer_id = parseInt(req.query.id); // Parse the customer ID from the query parameter
  //console.log(customer_id);
  connection.query("SELECT * FROM users u join customer_details cd on u.user_id = cd.user_id where customer_id = ?",[customer_id], function (error, results, fields) {
...
app.get("/get-single-writer-details",ensureAuthenticated, function (req, res) {
  const writer_id = parseInt(req.query.id); 
  const loggedInUserType = req.session.loggedInUserType;
...
app.get('/export-to-excel',ensureAuthenticated, function (req, res) {
  const loggedInUserType = req.session.loggedInUserType;
  const function_id = parseInt(req.query.id); 
  const functions = req.session.functions || [];
      ...
      switch (loggedInUserType) {
        case 1:
          ...
          break;
        case 2:
            if (req.session.function_id !== function_id) {
                return res.status(403).send("Forbidden");
            }
            break;
        case 3:
          const isFunctionAllowed = functions.some(func => func.function_id === function_id);
  ...
  select vd.visitor_name,vd.visitor_husband_or_wifename,vd.visitor_profession,
  vd.visitor_phone_no,vd.visitor_address,vd.visitor_town_or_village,vd.visitor_city,vp.visitor_payment 
  from visitor_details vd
  join visitor_payments vp
  on vd.visitor_id = vp.visitor_id 
  where vp.function_id = ? and vp.is_delete = 0
  ORDER BY vd.visitor_city,vd.visitor_town_or_village;
  `;
...
app.get("/get-function-details-for-subadmin",requireSubAdminAuth, function (req, res) {
  //console.log("get single customer details function called ");
  const customer_id = parseInt(req.query.id); // Parse the customer ID from the query parameter
  console.log(customer_id);
  connection.query("SELECT * FROM function_details where customer_id = ? and is_delete = 0 ORDER BY CASE when updated_at > created_at then updated_at else created_at end desc;  ",[customer_id], function (error, results, fields) {
      if (error) {
        ...
        if (results.length === 0) {
            res.json({success: false, message: "Customer doesn't have any functions yet." });
        } else {
...
// =========================== FOR GETTING THE TOTAL PAYMENT FOR EACH WRITER ========================================================

app.get("/get-writers-payment-total", requireWriterAuth, function (req, res) {
  const function_id = parseInt(req.query.id); 
  const user_id = parseInt(req.query.user_id); 

  connection.query(
    "SELECT SUM(visitor_payment) as totalPayment from visitor_payments where user_id = ? and function_id = ? and is_delete = 0",
    [user_id,function_id],function (error, results, fields) {
      if (error) {
        console.error("Error fetching total payment:", error);
        res.status(500).send("Internal server error");
      } else {
        // Send the total customer count as JSON response
        res.json({ totalPayment: results[0].totalPayment });
      }
    }
  );
});
...
app.get("/get-writer-details-for-subadmin",requireSubAdminAuth, function (req, res) {
  // Query the database to fetch existing writer details
  const inserted_by = req.query.id; 

  connection.query("SELECT * FROM users u join writer_details wd on u.user_id = wd.user_id where u.is_delete = 0 and u.inserted_by = ? ORDER BY CASE WHEN wd.updated_at > u.updated_at THEN wd.updated_at ELSE u.updated_at END DESC",[inserted_by], function (error, results, fields) {
      if (error) {
          console.error("Error fetching writer details:", error);
          res.status(500).send("Internal server error");
      } else {
...
app.get("/get-writers-count", ensureAuthenticated, function (req, res) {
  // Query the database to fetch existing writer details
  const loggedInUserId = req.query.id; 
  ...
  connection.query(
    "SELECT COUNT(*) as totalWriters from users where user_type = 2 and inserted_by = ? and is_delete = 0",[loggedInUserId],
    function (error, results, fields) {
      if (error) {
...
//============================= for getting the all writer_details based on the phone_no of the writer ==========================
 ...
 app.get('/api/get-writer-details-by-id', ensureAuthenticated, (req, res) => {
  const writer_id = req.query.id; 
  const loggedInUserType = req.session.loggedInUserType;
  ...
  connection.query(
      'SELECT * FROM users u join writer_details wd on u.user_id = wd.user_id WHERE wd.writer_id = ?',
      [writer_id],
      (err, results) => {
          if (err) {
              console.error('Error querying database:', err);
...
// =================================================== GETTING ALL SUB ADMIN DETAILS ===========================================================
app.get("/get-sub-admin-details",requireAdminAuth, function (req, res) {
  // Query the database to fetch existing admin details
  connection.query("SELECT * FROM users u join admin_details ad on u.user_id = ad.user_id where u.is_delete = 0 ORDER BY CASE WHEN ad.updated_at > u.updated_at THEN ad.updated_at ELSE u.updated_at END DESC", function (error, results, fields) {
      if (error) {
...
//============================= for getting the all admin_details based on the id of the admin ===================================================

app.get('/api/get-sub-admin-details-by-id', ensureAuthenticated, (req, res) => {
  ...
  connection.query(
      'select * from admin_details ad join users u on ad.user_id = u.user_id where ad.admin_id = ? ',
      [admin_id],
      (err, results) => {
          if (err) {
              console.error('Error querying database:', err);
...
// ======================================= ROUTE TO GET SINGLE SUB ADMIN DETAILS BASED ON ADMIN ID ===================================================

app.get("/get-single-sub-admin-details",requireAdminAuth, function (req, res) {
  const admin_id = parseInt(req.query.id); // Parse the customer ID from the query parameter
  console.log(admin_id);
  connection.query("SELECT * FROM users u join admin_details ad on u.user_id = ad.user_id where admin_id = ?",[admin_id], function (error, results, fields) {
      if (error) {
          console.error("Error fetching admin details:", error);
          res.status(500).send("Internal server error");
      } else {
...
app.get('/fetchVisitorData',ensureAuthenticated, (req, res) => {
...
app.get('/get-visitor-details', ensureAuthenticated, function (req, res) {
  ...
  connection.query(
      'select * from visitor_payments vp join visitor_details vd on vp.visitor_id = vd.visitor_id join users u on u.user_id = vp.user_id where vp.function_id = ? AND vp.is_delete = 0 ORDER BY CASE WHEN vp.updated_at > vp.created_at THEN vp.updated_at ELSE vp.created_at END DESC',
      [function_id],
      function (error, results, fields) {
          if (error) {
...
app.get('/api/get-visitor-details-by-id', ensureAuthenticated, (req, res) => {
  const visitor_id = req.query.id; 
  const visitor_payment_id = req.query.visitor_payment_id;

  connection.query(
      'select * from visitor_details vd join visitor_payments vp on vd.visitor_id = vp.visitor_id join users u on vp.user_id = u.user_id where vd.visitor_id = ? AND vp.visitor_payment_id = ?',
      [visitor_id,visitor_payment_id],
      (err, results) => {
          if (err) {
              console.error('Error querying database:', err);
              return res.status(500).json({ error: 'Internal server error' });
...
app.get('/get-single-function-details/:function_id', requireWriterAuth, (req, res) => {
  const functionId = req.params.function_id;

  const sql = `
      SELECT
      u.name,fd.function_name,fd.function_address,
      fd.function_city,u.phone_no,cd.customer_occupation,
      cd.customer_husband_or_wifename
    FROM function_details fd
    JOIN customer_details cd ON fd.customer_id = cd.customer_id
    JOIN users u ON cd.user_id = u.user_id
    WHERE fd.function_id = ?;
  ...
  // Execute the query with the function_id as a parameter
  connection.query(sql, [functionId], (err, results) => {
    if (err) {
      console.error('Error fetching function details: ' + err.message);
      res.status(500).json({ success: false, message: 'Failed to fetch function details' });
      return;
    }
    ...
    if (results.length > 0) {
      ...
      res.status(404).json({ success: false, message: 'Function details not found' });
    }
  });
});
...
app.get("/sub_admin_customer_details.html", requireSubAdminAuth, function (req, res) {
  res.sendFile(__dirname + "/sub_admin_customer_details.html");
});
...
app.get("/sub_admin_writer_details.html", requireSubAdminAuth, function (req, res) {
  res.sendFile(__dirname + "/sub_admin_writer_details.html");
});

app.get("/agent_details.html", requireAdminAuth, function (req, res) {
  res.sendFile(__dirname + "/agent_details.html");
});
...
app.get("/customer_dashboard", requireCustomerAuth, function (req, res) {
  res.sendFile(__dirname + "/customer_dashboard.html");
});
...
app.get("/sub-admin-visitor-entry", requireSubAdminAuth, function (req, res) {
  res.sendFile(__dirname + "/sub_admin_visitor_entry_page.html");
});
